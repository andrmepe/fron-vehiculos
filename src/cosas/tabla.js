...
const Tabla=()=>{
    ...
    let obtenerDatos=async()=>{
        try{
            const{data:response}=await axios.get('http://localhost:3030/autos')
            setData(response)
        }catch(error){
    ...
    const [precio,setPrecio]=useState('')
        ...
        if(nombre===''|| año_fabricacion===''||cantidad===''||precio===''){
            ...
            if (!Mostrar){
            axios.post('http://localhost:3030/agregar-auto',{
                nombre:nombre,
    ...
    const actualizar=()=>{
        if (nombre===''|| año_fabricacion===''||cantidad===''||precio===''){
            ...
            axios.put(`http://localhost:3030/actualizar-auto/${id}`,{
                nombre:nombre,
                año_fabricacion:año_fabricacion,
                cantidad:cantidad,
                precio:precio
